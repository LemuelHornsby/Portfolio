#!/usr/bin/env python3
"""
Interactive Dashboard for Simulation_realtime.py (Flow & ΔP) — v3
------------------------------------------------------------------
- Start/Stop controls
- Two live charts (Flow & ΔP): measured vs simulated OVERLAID
- Pump schematic with flow-reactive arrow
- Co-Simulation if FMU supports it; otherwise true Model-Exchange fallback by
  re-running simulate_fmu() over the accumulated inputs (distinct simulated outputs).
- NI-DAQ if available; otherwise DEV_MODE dummy signals.

Run:
  python Simulation_realtime_dashboard_v3.py
"""
...
class FMURunner:
    """Co-Sim if available; else re-simulate with accumulated inputs so sim != measured."""
    def __init__(self, fmu_path: str, dt: float):
        ...
        # history for ME resim
        self.hist_t = []
        self.hist_rpm = []
        self.hist_flow_target = []
    ...
    def step(self, rpm, flow_target, pin, pout):
        ...
        if self.mode == "cs" and self.fmu is not None:
            try:
                ...
                print(f"[FMU] step error (CS): {e}")
                self.t += self.dt

        elif self.mode == "me" and HAS_FMPY:
            next_t = (self.hist_t[-1] + self.dt) if self.hist_t else 0.0
            self.hist_t.append(next_t)
            self.hist_rpm.append(rpm)
            self.hist_flow_target.append(flow_target)

            t_vec = np.array(self.hist_t, dtype=float)
            inputs = np.zeros(t_vec.shape, dtype=[('time', 'f8'),
                                                  (FMU_INPUT_FLOW_TARGET, 'f8'),
                                                  (FMU_INPUT_PUMP_SPEED,  'f8')])
            inputs['time'] = t_vec
            inputs[FMU_INPUT_FLOW_TARGET] = np.array(self.hist_flow_target, dtype=float)
            inputs[FMU_INPUT_PUMP_SPEED]  = np.array(self.hist_rpm, dtype=float)

            try:
                res = simulate_fmu(self.fmu_path, start_time=0.0, stop_time=float(t_vec[-1] if t_vec[-1] > 0 else self.dt),
                                   input=inputs)
                if FMU_OUT_FLOW_RATE in res.dtype.names:
                    flow_sim = float(res[FMU_OUT_FLOW_RATE][-1])
                if FMU_OUT_DP in res.dtype.names:
                    dp_sim = float(res[FMU_OUT_DP][-1])
            except Exception as e:
                print(f"[FMU] simulate_fmu error (ME): {e}")
...
class Dashboard(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Pump Digital Twin — Flow & ΔP Dashboard (Overlay: Measured vs Simulated)")
        ...
        self.line_flow_meas, = self.ax_flow.plot([], [], label="Measured", linestyle="--", marker="", linewidth=1.8)
        self.line_flow_sim,  = self.ax_flow.plot([], [], label="Simulated", linestyle="-",  marker="", linewidth=2.0)
        ...
        self.line_dp_meas, = self.ax_dp.plot([], [], label="Measured", linestyle="--", marker="", linewidth=1.8)
        self.line_dp_sim,  = self.ax_dp.plot([], [], label="Simulated", linestyle="-",  marker="", linewidth=2.0)
